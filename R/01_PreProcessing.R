...
filename <- paste0(path, "Resultaten_2012.xls")
...
filename <- paste0(path, "Resultaten_2014.xls")
...
filename <- paste0(path, "Resultaten_2016.xls")
...
filename <- paste0(path, "Resultaten_2018.xls")
...
df.resp <- rbind(df.2012flat, df.2014flat, df.2016flat, df.2018flat)

# Enkele correcties

# 'jonger dan 30 jaar' in 2012-2016 moet eigenlijk 'jonger dan 31 jaar' zijn.
# In de enquete van 2018 is deze categorie correct.
...
# In 2018 is er plots een categorie 'Weet het niet' ipv 'weet niet'
...
# In 2018 is de categorie 'Noch akkoord, noch niet akkoord' soms
# met een komma en soms niet. Aanpassen dat alle antwoorden
# voor alle jaren steeds met een komma zijn
df.resp <- mutate(df.resp,
                  Resp = ifelse(Resp == "Noch akkoord noch niet akkoord",
                                "Noch akkoord, noch niet akkoord",
                                Resp))

# In 2018 zijn de levels voor de antwoordcategorie over 'belangrijkheid' plots
# anders dan in 2012 en 2014. De antwoorden van deze laatste worden omgezet in
